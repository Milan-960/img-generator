...
class ImgGenerator extends React.Component {
 ...
 //Download PNG image
  handleRemoveBg= () => {
    ...
    fd.append("image", this.state.image, 'demo');
    axios
      .post(
        "https://inpixio-remove-bg-zceht2uy2q-ey.a.run.app/image/remove_bg",
        fd
      )
      .then(function(res) { /*setLink("data:image/png;base64," + res.data.image)*/ 
     setImage(randomImg: URL.createObjectURL(res.data.image))})
